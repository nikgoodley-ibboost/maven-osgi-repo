...
public class ArtifactMetadataContentTest {
	...
	public void canSerializeMetaData() throws IOException, SAXException {
		...
		ArtifactMetadataContent content = new ArtifactMetadataContent(data, versions.last(), versions.last());

		// assertEquals("a","b");

		ContentTestUtil.assertGeneratesXMLContent("<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + "<metadata>"
				+ "<groupId>com.crsn</groupId>" + "<artifactId>test</artifactId>" + "<versioning><latest>3</latest>"
				+ "<release>3</release>" + "<versions>" + "<version>1</version>" + "<version>2</version>"
				+ "<version>3</version>" + "</versions>" + "</versioning>" + "</metadata>", content);
	}
